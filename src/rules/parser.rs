...
impl RuleParser {
    ...
    pub fn parse_rule_file<P: AsRef<Path>>(&self, path: P) -> Result<Vec<Rule>> {
        ...
        // 如果启用了缓存，尝试从缓存获取
        if self.use_cache {
            if let Some(cache) = CacheManager::global().get_file_cache::<String, (RuleFileMeta, Vec<Rule>)>("default") {
        ...
        let file = File::open(path)
            .context(format!("无法打开规则文件: {:?}", path))?;
        let reader = BufReader::new(file);
        
        let rules: Vec<Rule> = serde_json::from_reader(reader)
            .context(format!("无法解析规则文件: {:?}", path))?;
    ...
    pub fn parse_all_rules(&self) -> Result<Vec<Rule>> {
        ...
        for path in json_files {
            if path.file_name().map_or(false, |name| name == "default.json") {
