...
use crate::core::cache::{FileCache, MemoryCache, LruMemoryCache};
...
impl CacheManager {
        ...
        V: Clone + Send + Sync + Serialize + for<'de> Deserialize<'de> + 'static,
    {
        ...
        caches.insert(name.to_string(), cache as Arc<dyn std::any::Any + Send + Sync>);
    }
        ...
        V: Clone + Send + Sync + Serialize + for<'de> Deserialize<'de> + 'static,
    {
        ...
        caches.get(name).and_then(|cache| {
            cache.clone().downcast::<FileCache<K, V>>().ok()
        })
    }
        ...
        V: Clone + Send + Sync + 'static,
    {
        ...
        caches.get(name).and_then(|cache| {
            cache.clone().downcast::<MemoryCache<K, V>>().ok()
        })
    }
        ...
        V: Clone + Send + Sync + 'static,
    {
        ...
        caches.get(name).and_then(|cache| {
            cache.clone().downcast::<LruMemoryCache<K, V>>().ok()
        })
    }
